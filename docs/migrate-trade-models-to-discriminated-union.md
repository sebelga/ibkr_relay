...
- **The sign convention must survive the split — this is the easiest thing to silently drop.** `Fill` and `Trade` each carry a `@model_validator(mode="after")` calling `_apply_sign_convention`, which enforces the signed-delta contract (`volume` buy +/sell −, `cost` buy −/sell +, `fee` always +). The skeletons in *Target state* above omit it for brevity. Put the validator on `_BaseFill` (and the `Trade` equivalent) so every variant inherits it — declaring it per-variant invites one to be forgotten. Verify with a sell-side test **per variant**: a buy-only fixture passes under either convention, which is precisely how a positive-volume `sell` once reached production. See the sign convention section in [services/relays/CLAUDE.md](../services/relays/CLAUDE.md).
