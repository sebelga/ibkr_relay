...
- **`volume` and `cost` are signed deltas — adapters must never sign them.** `volume` is a position delta (buy +, sell −); `cost` is the opposite-signed cash delta (buy −, sell +); `price` is never signed. Forward the broker's native values; `_apply_sign_convention` (a `model_validator` on `Fill`/`Trade` in `services/shared/models.py`) re-signs from `side` and is idempotent. Brokers disagree — IBKR Flex signs `quantity` our way but reports `cost` as an inverted cost-basis delta, while the IBKR bridge and both Kraken paths send unsigned magnitudes — so centralising is what keeps a new adapter from inventing a fourth convention. **Every adapter needs a sell-side test asserting `volume < 0` and `cost > 0`**: a buy-only fixture passes under either convention, which is how a positive-volume `sell` reached production. Full table in [services/relays/CLAUDE.md](services/relays/CLAUDE.md).
